...
- **agent-exec strong-model escalation (Phase EXEC Tier D) — IMPLEMENTED but DISABLED fleet-wide.** A task tagged `metadata.exec="agent"` + `metadata.skills=["agent-exec"]` would spawn `pi --print` with the strong local model (`qwen3.5:35b-a3b`) + full tools + a dedicated capable-agent prompt, so a capable model runs the agentic decision loop locally (`src/agent-exec-handler.ts`; `BridgeConfig` gains `agentExec*`; opt-in `maxQueue` on the bridge). 13 TDD tests, 336/336 green; RULE 23 deepseek PASS + kimi CONDITIONAL (findings applied). **Verification found the code path works (minimal task ran 35B + returned real output) but `qwen3.5:35b-a3b` is too heavy for A2A agent-loops on 32GB nodes — peaks ~25GB/~6GB avail + `OLLAMA_KEEP_ALIVE=0` reload churn OOMs the node on multi-step hard tasks (fnet4 rebooted mid-task).** DISABLED via `bridge_agentExecEnabled=false` so nodes explicitly-fail agent-exec tasks instead of crashing. Re-enable blocked on a lighter capable model (e.g. `qwen3.5:14b`) or more RAM. Until then, keep hard agentic tasks on the orchestrator's cloud models.
